...
- **10 Podstrona i motyw** — auto-tworzenie „Nasze motory" + wpięcie w menu: block (`wp_navigation`, z pominięciem, gdy jest `page-list` — inaczej duplikat), classic (menu location) oraz motywy z menu „na sztywno" przez filtr `kk_menu_items`; styl dziedziczy fonty/kolory motywu (`currentColor`, `color-mix`).
